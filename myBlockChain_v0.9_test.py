...
PORT_NUMBER = 8096
g_txFileName = "txData.csv" #거래데이터
g_bcFileName = "blockchain.csv" #블록체인 정보
g_nodelstFileName = "nodelst.csv" #노드 정보(서버ip,port,커넥션실패숫자)를 저장하는 파일
    ...
    try:
        with open(g_bcFileName, 'r',  newline='') as file:
            ...
            for line in blockReader:
                if blockReader.line_num == last_line_number: #line_num : 블록체인 라인당 번호가 하나씩 들어옴
...
def addNode(queryStr): #http://localhost:8099/node/addNode?127.0.0.1:8096  key:127.0.0.1, value: 8096
    # save
    txDataList = []
...
def readNodes(filePath):
    ...
    try:
        with open(filePath, 'r',  newline='') as file:
            txReader = csv.reader(file)
            for row in txReader:
                line = [row[0],row[1]] #row[0] : ip , row[1] : port
...
def broadcastNewBlock(blockchain):
    ...
    reqBody = [] #블록체인 정보를 딕셔너리형태로 붙여서 넣음
    for i in blockchain:
...
def compareMerge(bcDict):
    ...
    heldBlock = [] #내가가진 블록체인 정보를 담을 곳
    bcToValidateForBlock = [] #밖에서 들어온 블록체인 정보를 담을 곳
    ...
    try:
        with open(g_bcFileName, 'r',  newline='') as file:
            ...
            for line in blockReader:
                block = Block(line[0], line[1], line[2], line[3], line[4], line[5])
                heldBlock.append(block) #내가가진 블록체인 정보를 담는다.
    ...
    #if it fails to read block data  from db(csv)
    if len(heldBlock) == 0 : #내가 가진 데이터가 제대로 안들어왔을 때,
        print("fail to read")
        return -2

    # transform given data to Block object
    for line in bcDict: #밖에서 받은 데이터 정보를 받음.
        # print(type(line))
        # index, previousHash, timestamp, data, currentHash, proof
        block = Block(line['index'], line['previousHash'], line['timestamp'], line['data'], line['currentHash'], line['proof'])
        bcToValidateForBlock.append(block)
    ...
    # compare the given data with genesisBlock
    if not isSameBlock(bcToValidateForBlock[0], heldBlock[0]): #처음 블록이 같은지 확인
        print('Genesis Block Incorrect')
        return -1

    # check if broadcasted new block,1 ahead than > last held block

    if isValidNewBlock(bcToValidateForBlock[-1],heldBlock[-1]) == False: #배열의 맨마지막값을 가지고옴, 선후행관계검사

        # latest block == broadcasted last block
        if isSameBlock(heldBlock[-1], bcToValidateForBlock[-1]) == True: #맨마지막값 두개가 같을 때,
            print('latest block == broadcasted last block, already updated')
            return 2
        # select longest chain
        elif len(bcToValidateForBlock) > len(heldBlock): #들어온블록이 내가가진 것보다 길 때,
            # validation
            ...
            # [END] save it to csv
            return 1
        elif len(bcToValidateForBlock) < len(heldBlock): #내가가진 블록이 길 때,
            # validation
            #for i in range(0,len(bcToValidateForBlock)):
            ...
            #        return -1
            tempBlocks = [bcToValidateForBlock[0]]
            for i in range(1, len(bcToValidateForBlock)): #내가 가진 블록이 길지만, 보내준 블록을 검증해줌,
                if isValidNewBlock(bcToValidateForBlock[i], tempBlocks[i - 1]):
                    tempBlocks.append(bcToValidateForBlock[i])
                else:
                    return -1
...
class myHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if None != re.search('/block/*', self.path):
            ...
            self.send_header('Content-type', 'application/json')
            self.end_headers()
            if None != re.search('/node/addNode', self.path): #http://localhost:8099/node/addNode?127.0.0.1:8096
